...
public class ConfigScreen extends Screen {
    ...
    private void refreshWidgets() {
        ...
        switch (currentTab) {
            ...
            case 4: // FAVORITES
                ...
                for (int i = favScrollOffset; i < Math.min(fvs.size(), favScrollOffset + 8); i++) {
                    ...
                    if (renamingEntry != null && renamingEntry.url.equals(e.url)) {
                        ...
                        addDrawableChild(ButtonWidget.builder(Text.literal("✔"), b -> { e.label = renameField.getText(); renamingEntry = null; SonicPulseConfig.save(); refreshWidgets(); }).dimensions(contentX + contentW - 18, rY, 18, rowH).build());
                    } else {
            ...
            case 5: // RADIO
                ...
                break;
            case 6: // LOCAL
                for (int i = localScrollOffset; i < Math.min(localFiles.size(), localScrollOffset + 8); i++) {
                    File fl = localFiles.get(i);
                    addDrawableChild(ButtonWidget.builder(Text.literal("♫ " + textRenderer.trimToWidth(fl.getName(), contentW-25)), b -> { SonicPulseClient.getEngine().playTrack(fl.getAbsolutePath(), fl.getName(), "Local"); refreshWidgets(); }).dimensions(contentX, tabY + ((i - localScrollOffset) * rowH), contentW - 15, rowH).build());
                }
                break;
        }
    }
    ...
    private void scanLocalFiles() {
        localFiles.clear(); String path = config.localMusicPath.isEmpty() ? MinecraftClient.getInstance().runDirectory.toPath().resolve("sonicpulse/music").toString() : config.localMusicPath;
        File dr = new File(path); if (dr.exists() && dr.isDirectory()) { File[] fls = dr.listFiles((d, n) -> { String nm = n.toLowerCase(); return nm.endsWith(".mp3") || nm.endsWith(".wav"); }); if (fls != null) Collections.addAll(localFiles, fls); }
    }
    ...
    @Override public boolean mouseScrolled(double mx, double my, double h, double v) {
        ...
        if (currentTab == 6) localScrollOffset = Math.max(0, Math.min(localFiles.size() - 8, localScrollOffset + d));
        refreshWidgets(); return true;
    }

    @Override public void render(DrawContext context, int mx, int my, float d) {
        int x = (width - BOX_WIDTH) / 2, y = (height - BOX_HEIGHT) / 2;
        int contentX = x + SIDEBAR_WIDTH + 10, tabY = y + 42;
