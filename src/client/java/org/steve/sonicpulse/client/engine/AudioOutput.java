...
public class AudioOutput {
    ...
    public AudioOutput(AudioPlayer player) {
        this.player = player;
    }
    ...
    private void updateFilterCoefficients() {
        ...
        boolean submerged = MinecraftClient.getInstance().player != null
    ...
    private void setupLowShelf(float freq, float db) {
        ...
        a1 = -2 * ((A - 1) + (A + 1) * cs);
        a2 = (A + 1) + (A - 1) * cs - 2 * (float) Math.sqrt(A) * alpha;
        b0 /= a0;
        b1 /= a0;
        b2 /= a0;
        a1 /= a0;
    ...
    private void setupHighShelf(float freq, float db) {
        ...
        t_a1 = 2 * ((A - 1) - (A + 1) * cs);
        t_a2 = (A + 1) - (A - 1) * cs - 2 * (float) Math.sqrt(A) * alpha;
        t_b0 /= ta0;
        t_b1 /= ta0;
        t_b2 /= ta0;
        t_a1 /= ta0;
        ...
        while (running) {
            try {
                ...
                if (dataToPlay != null) {
                    ...
                    boolean actuallyMuffle = cfg.underwaterMuffle && MinecraftClient.getInstance().player != null
                            && MinecraftClient.getInstance().player.isSubmergedInWater();
                    ...
                    for (int i = 0; i < dataToPlay.length; i += 4) {
                        ...
                        dataToPlay[i] = (byte) (outL & 0xFF);
                        dataToPlay[i + 1] = (byte) ((outL >> 8) & 0xFF);
                        dataToPlay[i + 2] = (byte) (outR & 0xFF);
                        dataToPlay[i + 3] = (byte) ((outR >> 8) & 0xFF);
                    }
    ...
    private void updateAmplitudes(byte[] data) {
        ...
        for (int s = 1; s <= m; s++) {
            ...
            for (int k = 0; k < n; k += m2) {
                ...
                for (int j = 0; j < m1; j++) {
                    ...
                    float uR = realBuffer[k + j];
                    float uI = imagBuffer[k + j];
                    realBuffer[k + j] = uR + tR;
                    imagBuffer[k + j] = uI + tI;
                    realBuffer[k + j + m1] = uR - tR;
                    imagBuffer[k + j + m1] = uI - tI;
                    float nWR = wR * wmR - wI * wmI;
                    wI = wR * wmI + wI * wmR;
                    wR = nWR;
                }
            }
        ...
        for (int i = 0; i < 32; i++) {
            ...
            if (val > amplitudes[i])
                amplitudes[i] = Math.min(1f, val);
            else
                amplitudes[i] = amplitudes[i] * 0.75f + Math.min(1f, val) * 0.25f;
        }
    }
}