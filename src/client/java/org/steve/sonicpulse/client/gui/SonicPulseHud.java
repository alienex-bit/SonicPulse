...
public class SonicPulseHud {
    ...
    private Text buildTag(String icon, String name) {
        return Text.empty().append(Text.literal("[ ").setStyle(ROBOTO)).append(Text.literal(icon))
    ...
    public void render(DrawContext context, boolean isPreview, int previewX, int previewY) {
        ...
        if (!cfg.hudVisible || (!isPreview && client.currentScreen != null))
            return;
        ...
        for (int i = 0; i < activeSlots.size(); i++) {
            ...
            if (slotType == 1) { // LOGO
                ...
                if (engine != null && engine.isBuffering()) {
                    ...
                    context.drawText(textRenderer, style("§7" + bottomText), slotCenterX - (bW / 2), 18, 0xFFFFFFFF,
                            false);
                }
                // STANDARD MARQUEE ONCE PLAYING
                    ...
                    int sTxtW = (int) (textRenderer.getWidth(safeTrackNameCache) * tScale);
                    context.getMatrices().push();
                    if (sTxtW > maxW) {
                        if (System.currentTimeMillis() - lastMarqueeUpdate > 100) {
                            int offset = (int) ((System.currentTimeMillis() / 150) % marqueeLenCache);
                            currentScrolledText = textRenderer.trimToWidth(marqueeCache.substring(offset),
                                    (int) (maxW / tScale));
                            lastMarqueeUpdate = System.currentTimeMillis();
                        }
                        context.getMatrices().translate(slotCenterX - (maxW / 2f), 11, 0);
                        context.getMatrices().scale(tScale, tScale, 1f);
                        context.drawText(textRenderer, style(currentScrolledText), 0, 0, 0xFFFFFFFF, false);
                    } else {
                        context.getMatrices().translate(slotCenterX - (sTxtW / 2f), 11, 0);
                        context.getMatrices().scale(tScale, tScale, 1f);
